...
count2 = 0
obst_count = 1
mymap = np.array([[0 for x in range(1700)] for y in range(1400)])
def pose_callback( pose_data ):
    ...
    global xreal
    global laser_scan
    xreal[0,0] = pose_data.x
    xreal[1,0] = pose_data.y
    xreal[2,0] = pose_data.theta
    #print xreal
# go through each point the lidar returned and turn it into a set of filled and unfilled points
    #mymap = [[0 for x in range(1700)] for y in range(1700)]
    for i in range(0,360):
        laser = laser_scan.ranges[i]
        if isinf(laser) == False:
            phi = i*laser_scan.angle_increment + laser_scan.angle_min
# calculate location in local coordinates
            #print xreal[0,0]
            #print xreal[1,0]
            #print xreal[2,0]
            xloc = (xreal[0,0]+laser*cos(xreal[2,0]+phi))*10
            #print xloc
            xloc = int(xloc)
            #print xloc
            yloc = int((xreal[1,0]+laser*sin(xreal[2,0]+phi))*10)
            # check a spot 45 degrees in front to see if it's already filled
            if i == 135 and mymap[xloc][yloc] > 0:
                completed_loop = True
                print "completed Loop"
                #obst_count = obst_count + 1
                break
                # publish the command to run brushfire
            if i > 30 and i < 90:
                mymap[xloc][yloc] = obst_count
            elif mymap[xloc][yloc] < 1:
                mymap[xloc][yloc] = -1

    if count2 % 5 == 0:
        val = 15 
        for j in range(-val-1,val+2):
            for i in range(-val-1,val+2):
                mymapval = mymap[int(xreal[0]*10)+i*2][int(xreal[1]*10)-j*2]
                if i == -(val+1) or i == (val+1) or j == -(val+1) or j == (val+1):
                    sys.stdout.write('O ')
                elif mymapval>0:
                    sys.stdout.write(str(mymapval))
                    sys.stdout.write(' ')
                elif mymapval == -1:
                    sys.stdout.write('X ')
                else:
...
def omega_callback( omega_data ):
    ...
    global x, xprev, u
    global P
    wcom1new = omega_data.data[0]
    wcom2new = omega_data.data[1]
    count = count+1
