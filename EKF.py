...
def f(x, u, r, L, dt):
    f = np.array([[x[0] + r * dt / 2 * (u[0] + u[1]) * cos(x[2])],
...
def F(x, u, r, L, dt):
    f = np.array([[1, 0, -r * dt / 2 * (u[0] + u[1]) * sin(x[2])],
                  [0, 1, r * dt / 2 * (u[0] + u[1]) * cos(x[2])], [0, 0, 1]])
    return f
...
for k in range(1, n):
    R = np.matrix([[np.random.normal()], [np.random.normal()],
                   [np.random.normal()]])
    xhat[k] = f(xhat[k - 1, :, 0], u[k, :, 0], r, L, dt)
    #print(xhat[k])
    P[k] = np.dot(
        np.dot(F(xhat[k, :, 0], u[k, :, 0], r, L, dt), P[k - 1]),
        F(xhat[k, :, 0], u[k, :, 0], r, L, dt).T) + V
    #print(P[k])
    #K = np.linalg.solve((P[k]+W).T,P[k].T).T
    K = np.linalg.solve((np.dot(H, np.dot(P[k], H.T)) + W).T, np.dot(
        H, P[k].T)).T
    #print(K)
    #print(x[k])
    #print(W)
...
plt.plot(
    xhat[1:80, 0, 0], xhat[1:80, 1, 0], color='b', linestyle='-', marker='x')
plt.show()
